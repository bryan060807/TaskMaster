...
import { ArrowUpRight, Brain, Sparkles, Loader2, Trash2 } from 'lucide-react'
...
export default function BrainDumpView({
  ...
  const groupedTasks = useMemo(() => {
    const groups: Partial<Record<Category, Task[]>> = {}
  ...
  const moveToToday = async (taskId: string) => {
    ...
    setTasks((prev) => prev.map((t) => (t.id === taskId ? { ...t, scheduledDate: today } : t)))
  ...
  return (
    <div className="space-y-10">
      {(Object.entries(groupedTasks) as [Category, Task[]][]).map(([category, categoryTasks]) => (
        <section key={category} className="space-y-4">
          <div className="flex items-center gap-2 px-2">
            <div className={`w-2 h-2 rounded-full ${
              category === 'snhu' ? 'bg-blue-500' :
              category === 'home' ? 'bg-green-500' : 'bg-neutral-500'
            }`} />
